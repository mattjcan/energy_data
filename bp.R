...
p_g_fuel <- fuel_by_globe %>% 
  ...
  theme_mc + 
  theme(legend.position = c(0.15, 0.867), legend.text = element_text(size=7), legend.background = element_rect(fill = background), legend.key = element_rect(fill = background), legend.title = element_blank(), legend.key.size = unit(2.9, "mm")) +
...
p_g_india <- elec_gen_india %>% 
  ...
  geom_line(aes(color = fuel), size = 1) + 
  theme_mc + 
  theme(legend.position = c(0.15, 0.4), legend.text = element_text(size=7), legend.background = element_rect(fill = background), legend.key = element_rect(fill = background), legend.title = element_blank(), legend.key.size = unit(2.9, "mm")) +
...
p_r_gen <- map(unique(elec_gen_coal$region), f_r_demand)

names(p_r_gen) <- unique(elec_gen_coal$region)
...
p_c_gen <- map(unique(elec_gen_coal$Country), f_c_demand)

names(p_c_gen) <- unique(elec_gen_coal$Country)
